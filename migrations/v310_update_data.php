	...
	public function abbc3_bbcode_data()
	{
		return array(
			'BBvideo' => array(
				...
				'first_pass_match'		=> '!\[bbvideo\=([0-9]+),([0-9]+)\](?:([a-z][a-z\d+\-.]*:/{2}(?:(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})+|[0-9.]+|\[[a-z0-9.]+:[a-z0-9.]+:[a-z0-9.:]+\])(?::\d*)?(?:/(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})*)*(?:\?(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?(?:#(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?)|(www\.(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})+(?::\d*)?(?:/(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})*)*(?:\?(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?(?:#(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?))\[/bbvideo\]!ie',
				'first_pass_replace'	=> '\'[bbvideo=${1},${2}:$uid]\'.$this->bbcode_specialchars((\'${3}\') ? \'${3}\' : \'http://${4}\').\'[/bbvideo:$uid]\'',
				'second_pass_match'		=> '!\[bbvideo\=([0-9]+),([0-9]+):$uid\](?i)((?:[a-z][a-z\d+\-.]*:/{2}(?:(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})+|[0-9.]+|\[[a-z0-9.]+:[a-z0-9.]+:[a-z0-9.:]+\])(?::\d*)?(?:/(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})*)*(?:\?(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?(?:#(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?)|(?:www\.(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})+(?::\d*)?(?:/(?:[a-z0-9\-._~\!$&\'()*+,;=:@|]+|%[\dA-F]{2})*)*(?:\?(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?(?:#(?:[a-z0-9\-._~\!$&\'()*+,;=:@/?|]+|%[\dA-F]{2})*)?))(?-i)\[/bbvideo:$uid\]!s',
				'second_pass_replace'	=> '<a href="${3}" class="bbvideo" data-bbvideo="${1},${2}">${3}</a>',
			),
			'glow=' => array(
				'bbcode_tag'			=> 'glow=',
			...
			'fade' => array(
				...
				'second_pass_replace'	=> '<span class="fadeEffect">${1}</span>',
			),
			'mod=' => array(
				'bbcode_tag'			=> 'mod=',
				...
				'second_pass_replace'	=> '<table class="ModTable"><tr><td class="exclamation" rowspan="2">&nbsp;!&nbsp;</td><td class="rowuser">${1} {L_MESSAGE}:</td></tr><tr><td class="rowtext">${2}</td></tr></table>',
			),
			'spoil' => array(
				'bbcode_tag'			=> 'spoil',
